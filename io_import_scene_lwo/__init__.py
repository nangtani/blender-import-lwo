...
def load_lwo(filename,
    ...
    build_objects(layers, surfs, clips, tags, name, ADD_SUBD_MOD, SKEL_TO_ARM, USE_EXISTING_MATERIALS)
            ...
            if block_type == b'IMAP':
                ...
                while suboffset < subchunk_len:
                    ...
                    if subsubchunk_name == b'OPAC':
                        opactype, = struct.unpack(">H", surf_bytes[offset+suboffset:offset+suboffset+2])
                    ...
                    if subsubchunk_name == b'IMAG':
                        texture.clipid, = struct.unpack(">H", surf_bytes[offset+suboffset:offset+suboffset+2])
...
def build_objects(object_layers, object_surfs, object_clips, object_tags, object_name, add_subd_mod, skel_to_arm, use_existing_materials):
